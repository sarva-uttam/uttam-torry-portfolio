...
| Privacy — PII on site | Blocked | owner published full home address (3 pages + CV PDF) | **not changed** — owner content; see finding V-1, prepared diff in residual-risks | `grep` locates all 4 occurrences | Physical-safety / doxxing risk until the owner decides |
| PDF metadata | Already protected | `Uttam-Torry-CV-2026.pdf` — Author/Title stripped (`anonymous` / `untitled`), no XMP | none | `pdfinfo` output | The address is in the **visible CV body** (not metadata) — part of V-1 |
